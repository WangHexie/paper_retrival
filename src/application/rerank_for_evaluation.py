...
class BertRerankApplication:
    def __init__(self, model_name, k=60, prediction=False, device="cuda:1"):
        ...
        self.rerank_model = BertRankSE(model_name, initial_load=True, device=device)
    ...
    def train(self):
        ...
        train_pubs_string, test_pubs_string, train_user_prediction, test_user_prediction, train_labels, test_labels = train_test_split(
            self.pubs_string, user_predictions, self.labels, test_size=200, shuffle=True, random_state=8888)
...
if __name__ == '__main__':
    BertRerankApplication("scibert_longlonglong", k=60, prediction=False, device="cuda:0").train()
